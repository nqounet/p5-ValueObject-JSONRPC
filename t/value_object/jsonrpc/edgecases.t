...
subtest 'edge cases' => sub {
  ...
  subtest 'Error data accepts JSON values and compares deeply' => sub {
    ...
    my $d1 = [1, {x => [2,3]}, 's'];
    my $e1 = ValueObject::JSONRPC::Error->new(code => $code, message => 'm', data => $d1);
    my $d2 = [1, {x => [2,3]}, 's'];
    my $e2 = ValueObject::JSONRPC::Error->new(code => $code, message => 'm', data => $d2);
    ok $e1->equals($e2), 'errors with equivalent arrayref data are equal';

    my $e3 = ValueObject::JSONRPC::Error->new(code => $code, message => 'm', data => 'scalar');
    my $e4 = ValueObject::JSONRPC::Error->new(code => $code, message => 'm', data => 'scalar');
  ...
  subtest 'Request rejects raw scalar id (must be Id instance)' => sub {
    ...
    like dies { ValueObject::JSONRPC::Request->new(method => $m, params => $p, id => 1) }, qr/JSON-RPC request id must be a ValueObject::JSONRPC::Id/, 'raw numeric id rejected';
    like dies { ValueObject::JSONRPC::Request->new(method => $m, params => $p, id => 'x') }, qr/JSON-RPC request id must be a ValueObject::JSONRPC::Id/, 'raw string id rejected';
    like dies { ValueObject::JSONRPC::Request->new(method => $m, params => $p, id => []) }, qr/JSON-RPC request id must be a ValueObject::JSONRPC::Id/, 'ref id rejected';
  };
