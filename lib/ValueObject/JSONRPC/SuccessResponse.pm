...
has 'jsonrpc' => (
  ...
  isa     => sub {
    my $v = $_[0];
    die qq{JSON-RPC response jsonrpc must be a ValueObject::JSONRPC::Version} unless ref $v && ref $v eq 'ValueObject::JSONRPC::Version';
  },
);
...
has 'result' => (
  ...
  isa      => sub {
    my $r = $_[0];
    die qq{JSON-RPC success response result must be a ValueObject::JSONRPC::Result} unless ref $r && ref $r eq 'ValueObject::JSONRPC::Result';
  },
);
