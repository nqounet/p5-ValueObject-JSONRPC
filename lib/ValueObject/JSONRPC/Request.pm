...
has 'jsonrpc' => (
  ...
  isa     => sub {
    my $v = $_[0];
    die qq{JSON-RPC request jsonrpc must be a ValueObject::JSONRPC::Version} unless ref $v && ref $v eq 'ValueObject::JSONRPC::Version';
  },
);
...
has 'method' => (
  ...
  isa      => sub {
    my $m = $_[0];
    die qq{JSON-RPC request method must be a ValueObject::JSONRPC::MethodName} unless ref $m && ref $m eq 'ValueObject::JSONRPC::MethodName';
  },
);
...
has 'params' => (
  ...
  isa => sub {
    ...
    die qq{JSON-RPC request params must be a ValueObject::JSONRPC::Params} unless ref $p && ref $p eq 'ValueObject::JSONRPC::Params';
  },
);
