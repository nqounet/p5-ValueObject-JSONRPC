...
has 'code' => (
  ...
  isa      => sub {
    ...
    die qq{JSON-RPC error code must be a ValueObject::JSONRPC::Code} unless ref $c && ref $c eq 'ValueObject::JSONRPC::Code';
  },
);
...
sub equals {
  ...
  if (ref $other) {
    ...
    if (ref $self->data || ref $other->data) {
      ...
      return (defined $self->data && defined $other->data && freeze($self->data) eq freeze($other->data)) ? 1 : 0;
    }
